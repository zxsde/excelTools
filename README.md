...
### V3.3 (2022-02-06)
 - 新功能：新增【更新PBC链接】的脚本(change_link_PBC)，可以批量修改PBC链接，功能和change_cells_link差不多，为了简单操作单独写一个脚本。
 - 新功能：新增【PBC分组】的脚本(group_PBC)，对 PBC/PRC 进行分组，把 all_PBC/all_PRC 目录下所有文件，按指定关系保存在各自相应的目录下。
 - 新功能：新增【校验数据准确性】的脚本(check_data_accuracy)，检查所有excel中，对指定工作表的行列数据进行检查。
 - 优化：部分变量、常量、注释的优化。
 - 修复：合并报表(merge_sheet)，有时候读取不到excel中数据，需要提前打开一次要读取的excel。

### V3.2 (2021-12-21)
 - 新功能：合并报表(merge_sheet)优化过滤功能，可以选择保留或删掉指定的数据。
 - 优化：部分变量、常量、注释的优化。

### V3.1 (2021-12-19)
 - 优化：资产负债表核对(diff_balance_sheet)适配特殊场景，prc的利润表如果比pbc的利润表多一行，那么比较的时候，
   prc表18行的值要减去17行，再和pbc表的17行进行对比，prc中19行的值要加上17行，然后和pbc的18行比较。
 - 优化：部分变量、常量、注释的优化。
 - 修复：更新公式脚本(change_cells_link)，若链接的目标文件不存在，则不更新该公式。

### V3.1 (2021-12-17)
 - 新功能：新增【资产负债表核对】的脚本(diff_balance_sheet)，对比两个excel中的资产负债表是否相同，需要指定对比的具体列。
   实际支持多excel多表对比，如文件夹A中所有excel和文件夹B下所有excel中，资产负债表/利润表是否相同。
 - 优化：部分变量、常量、注释的优化，提取公共方法。
...
### V2.5 (2021-10-17)
 ...
 - 优化：SKIP_ROWS 默认置为 0，以前通过 SKIP_ROWS 跳过开头的空行，现在通过 HEADER 直接指定表头所在的行，无需跳过。
 - 优化：往来账款差异(diff_account_current)读取写入数据时保留表头。
 - 优化：公共方法(检查文件/文件夹是否存在)提取到 commons_utils。
 - 修复：暂无
...
### V2.2 (2021-10-13)
 ...
 - 新功能：完善(Pandas简易教程)。
 - 优化：脚本(merge_excel)更名为脚本(merge_sheet)。
 - 优化：部分注释进行优化。
 - 修复：暂无
...
### V2.1 (2021-10-11)
 ...
 - 优化：拆分【批量拷贝 excel 到指定目录】为单独的脚本(copy_excel_to_target)，并新增分类功能，可以将不同类别的 excel 拷贝至不同目录。
 - 修复：暂无
